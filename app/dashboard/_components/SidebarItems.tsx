...
const SidebarItems: FC<SidebarItemsProps> = ({ route, onClick }) => {
    ...
    const currPath = pathname.replace("/dashboard", "");
    ...
    return (
        <Button
            onClick={onClick}
            variant="ghost"
            className={cn("flex w-full justify-start items-center gap-2 text-sm pl-6 bg-white text-blue-950",
                    isActive ? "font-semibold bg-blue-950 text-white group" : ""
                )}
        >
            <Icon className={cn("w-4 h-4", isActive ? "text-white group-hover:text-blue-950" : "text-blue-950")} />
            { label }
        </Button>
    );
};
